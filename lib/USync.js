...
class USync {
  ...
  async import(opts) {
    ...
    if (baseRepoName === this.parentRepoName) {
      throw new USyncError(`Can't import from parent repo`);
    }
   ...
   */
  async land(opts) {
    if (opts.baseRepoName !== this.parentRepoName) {
      throw new USyncError(`Can only land from parent repo`);
    }
    ...
    try {
      ...
      config.mapping[this.parentRepoName] = {'': ''};
