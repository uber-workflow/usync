    ...
    // import
    {
      ...
      const [globalUser, commitAuthor] = await Promise.all([
        ...
        childGit
          ...
          .then(commits => commits[commits.length - 1].author),
      ]);
      const commitMessage = await childGit
        ...
        .then(authors => {
          const trailers = authors
            ...
            .join('\n');
          // if message includes author trailers, only add one newline
          // between it and additional trailers
          const separator = /Co-authored-by: .+ <[^>]+>$/.test(message)
            ? '\n'
            : '\n\n';

          return trailers ? message + separator + trailers : message;
        });
